import socket

class Client:
    def __init__(self):
        self.target_host = 'localhost'
        self.target_port = 9998

    def runClient(self,data:'register or lgoin'):
        client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        client.connect((self.target_host, self.target_port))

        client.send(data)

        recvFromServer = client.recv(4096)

        recvFromServer= recvFromServer.decode('utf-8')
        print(type(recvFromServer))
        status , message,amount = recvFromServer.split(' ')

        if status == '201':
            print( status , message)
        elif status == '400':
            print(status,message)
        elif status == '301':
            print(status,message)
        elif status == '302':
            print(status,message)
        elif status == '303':
            print(status,message)
        elif status == '304':
            print(status,message)
        elif status == '200':
            print(status , message)
            print("Checking loginuser's amount: ",amount)
            print("you can do transition now ")
            #kzt
            print(" ")
            print(
                "Welcome User : Avilable options are:")
            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
            print("3) Transfer money")
            print("4) Deposit")
            print("5) Withdraw")
            print("6) Update customer's Name")
            print("7) Update customer's Password")
            print("8) Print all customers detail")
            print("9) Sign out")
            print(" ")
            option = int(input("Choose your option: "))
            if option==3:
                #message ka sender uname pr lr tr
                acceptName = input("Enter acceptName to transfer: ")
                amount = int(input("Enter amount to transfer: "))
                data = '3' + ' ' + message + ' ' + acceptName+' '+str(amount)
                data = bytes(data, 'utf-8')
                try:
                    self.runClient(data)
                    print('success')
                except Exception as err:
                    print(err)
            elif option == 4 :
                # message ka sender uname pr lr tr
                amount = int(input("Enter amount to deposit: "))
                data = '4' + ' ' + message + ' ' + 'acceptName' + ' ' + str(amount)
                data = bytes(data ,'utf-8')
                try :
                    self.runClient(data)
                    print('success')
                except Exception as err :
                    print(err)
            
            elif option == 5 :
                # message ka sender uname pr lr tr
                amount = int(input("Enter amount to withdraw: "))
                data = '5' + ' ' + message + ' ' + 'acceptName' + ' ' + str(amount)
                data = bytes(data ,'utf-8')
                try :
                    self.runClient(data)
                    print('success')
                except Exception as err :
                    print(err)
            
            elif option == 6 :
                # message ka sender uname pr lr tr
                newName = input("Enter newName to change: ")
                data = '6' + ' ' + message + ' ' + newName + ' ' + "amount"
                data = bytes(data ,'utf-8')
                try :
                    self.runClient(data)
                    print('success')
                except Exception as err :
                    print(err)

        client.close()

    def option(self):
        option = input("[********]Welcome from KZT's own DB[********]\n[+]Press-1 to Register\n[+]Press-2 to Login!")
        if option=='1':
            r_name = input("Enter username to Register=>:")
            r_pw = input("Enter password to Register=>:")
            r_pw2 = input("Enter password again to confirm=>:")
            if r_pw == r_pw2:
                r_amount = input("Enter amount to Register=>")
                r_allData=option+' '+r_name+' '+r_pw+' '+r_amount
                r_allData:bytes = bytes(r_allData,'utf-8')
                self.runClient(r_allData)
        elif option=='2':
            l_name = input("Enter username to Login=>:")
            l_pw = input("Enter password to Login=>:")
            l_allData = option+' '+l_name+' '+l_pw+' '+'l_amount'
            l_allData:bytes = bytes(l_allData,'utf-8')
            self.runClient(l_allData)

    

if __name__ == "__main__":
    tcpClient:Client=Client()
    while True:
        tcpClient.option()




